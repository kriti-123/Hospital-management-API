  ...
  try {
    ...
    if (!admin) {
      ...
      if (admin) {
        return res
          .status(200)
          .json({ message: "successfully inserted to databse", admin: admin });
      }
...
module.exports.updateProfile = async function (req, res, next) {
  try {
    ...
    if (data) {
      return res
        .status(200)
        .json({ message: "successfully updated to databse", adm: data });
...
module.exports.appointmentsList = async function (req, res, next) {
  try {
    ...
    if (appoint) {
      res
        .status(200)
        .json({ message: "succesfully got the data", appoint: appoint });
    } else {
...
module.exports.doctorList = async function (req, res, next) {
  try {
    ...
    if (doct) {
      res
        .status(200)
        .json({ message: "succesfully got the data", doctor: doct });
...
module.exports.viewFullProfile = async function (req, res, next) {
  try {
    ...
    if ((await Doctor.findById(id)) || (await Patient.findById(id))) {
      res
        .status(200)
        .json({ message: "successfull got the complete data", data: data });
    } else throw new AppError("errror occured", 404);
  } catch (e) {
    next(e);
  }
